		...
		foreach($data->globalObjects->tweets as $tweet) {
			...
			if (isset($tweet->entities->media)) {
				foreach($tweet->entities->media as $media) {
					$cleanedTweet = str_replace($media->url, '<a href="' . $media->expanded_url . '">' . $media->display_url . '</a>', $tweet->full_text);
				}
			} elseif (isset($tweet->entities->urls)) {
				foreach($tweet->entities->urls as $url) {
					$cleanedTweet = str_replace($url->url, '<a href="' . $url->expanded_url . '">' . $url->display_url . '</a>', $tweet->full_text);
				}
			} else {
