	...
	public function collectData()
	{
		...
		function makeTimestamp($relativeReleaseTime)
		{
			...
			foreach (explode(" ", $relativeReleaseTime) as $relativeTimeElement) {
				if (substr($relativeTimeElement, -1) == "d") {
					$relativeDays = substr($relativeTimeElement, 0, -1);
				}
				if (substr($relativeTimeElement, -1) == "h") {
					$relativeHours = substr($relativeTimeElement, 0, -1);
				}
			}
		...
		foreach ($showList as $showID) {
			...
			foreach ($html->find('.forum_header_border') as $element) {
				...
				if (empty($released->plaintext)) {
					continue;
				}

				// Filter entries that are older than 1 week
				if ($released->plaintext == '&gt;1 week') {
					continue;
				}
