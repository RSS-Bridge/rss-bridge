...
class GithubWorkflowBridge extends BridgeAbstract
{
    ...
    public function collectData()
    {
        ...
        foreach ($dom->find("div.Box-row") as $row) {
            $workflow_url =
                "https://github.com" . trim($row->find("a.d-flex", 0)->href);
            ...
            $raw_status = trim(
                $row->find("svg.octicon", 0)->getAttribute("aria-label"),
            ); // had to use that syntax instead of `->` because `aria-label` has a hyphen, which is apparently not valid in PHP
            ...
            $item["title"] = $this->getInput("workflow")
                ? "{$name}{$branch}{$run_number} {$status}"
                : "{$workflow} {$name}{$branch}{$run_number} {$status}";
            $item["timestamp"] = $timestamp;
            $item["author"] = $initiator;
            $item["content"] = $content;
            $item["categories"] = $this->getInput("workflow")
                ? [$branch, $initiator, $status]
                : [$workflow, $branch, $initiator, $status];
            $item["uid"] = $uid;
            $this->items[] = $item;
        }
    }
