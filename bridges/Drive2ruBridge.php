...
class Drive2ruBridge extends BridgeAbstract {
	...
	const DESCRIPTION = 'Лента новостей и тестдрайвов, бортжурналов по выбранной марке или модели (также работает с фильтром по категориям), блогов пользователей и публикаций по темам.';
	const PARAMETERS = array(
		'Новости и тест-драйвы' => array(),
		'Бортжурналы (По марке или модели)' => array(
			'url' => array(
	...
	private function getUserContent($url) {
		...
		foreach ($articles as $article) {
			...
			if($this->getInput('full_articles')) {
				$item['content'] = $this->addCommentsLink($this->adjustContent(getSimpleHTMLDOM($item['uri'])->find('div.c-post__body', 0))->innertext, $item['uri']);
			} else {
	...
	private function getNews() {
		...
		foreach ($articles as $article) {
			...
			$item['uri'] = urljoin(self::URI, $article->find('a.c-link--text', 0)->href);
			if($this->getInput('full_articles')) {
				$item['content'] = $this->addCommentsLink($this->adjustContent(getSimpleHTMLDOM($item['uri'])->find('div.article', 0))->innertext, $item['uri']);
			} else {
	...
	private function adjustContent($content) {
				...
				foreach ($content->getElementsByTagName('figcaption') as $attrs)
					$attrs->setAttribute('style', 'font-style: italic; font-size: small; margin-bottom: 75px; margin-left: 75px; margin-right: 120px');
