...
class CeskaTelevizeBridge extends BridgeAbstract {
  ...
  public function collectData() {
    ...
    if (!preg_match('/^(https:\/\/www\.ceskatelevize\.cz\/porady\/\d+-[a-z0-9-]+\/)(dily\/((nove|vysilani)\/)?)?$/', $url, $match)) {
    ...
    foreach ($html->find('.episodes-broadcast-content a.episode_list_item') as $element) {
      ...
      $item = array(
        ...
        'uri' => $itemUri,
        'content' => '<img src="https:' . $itemThumbnail->getAttribute('src') . '" /><br />' . $this->fixChars($itemContent->plaintext),
