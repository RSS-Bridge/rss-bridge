	...
	public function collectData(){
		...
		if($this->getInput('c') || $this->getInput('a')) {
			...
			foreach($array as $key => $element) {
				...
				$item['timestamp'] = strtotime(str_replace(array('Ig&aring;r', 'Idag'), array('yesterday', 'today'), current(explode("\t", str_replace("\t\t","\t",trim($element->find('div.post-heading', 0)->plaintext))))));
