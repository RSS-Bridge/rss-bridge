        ...
        if ($this->getInput('c') || $this->getInput('a') ) {
            ...
            foreach ($array as $key => $element) {
                ...
                if (isset($timestamp->plaintext)) {
                    $item['timestamp'] = strtotime(str_replace(array('Ig&aring;r', 'Idag'), array('yesterday', 'today'), trim($timestamp->plaintext)));
                }
            ...
            foreach ( $array as $key => $element ) {
                ...
                $item['timestamp'] = strtotime(
                    str_replace(
                        ...
                        current(explode("\t", str_replace("\t\t", "\t", trim($element->find('div.post-heading', 0)->plaintext))))
                    )
                );
