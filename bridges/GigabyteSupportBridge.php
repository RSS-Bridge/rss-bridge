...
class GigabyteSupportBridge extends BridgeAbstract
{
    ...
    public function getIcon() { return 'https://www.gigabyte.com/favicon.ico'; }

    public function getName()
    {
        $info = $this->getProductInfo();
        if (!$info) return parent::getName();
        ...
        return in_array($fragment, self::VALID_TYPES, true) ? $name . ' (' . ($fragment === 'bios' ? 'BIOS' : ucfirst($fragment)) . ')' : $name;
    }

    public function getURI() { return $this->getInput('url') ?: parent::getURI(); }

    private function getProductInfo(): ?array
    {
        if ($this->productInfo !== null) return $this->productInfo;
        $url = $this->getInput('url');
        if (!$url) return null;
        $parsedUrl = parse_url($url);
        $segments = array_values(array_filter(explode('/', trim($parsedUrl['path'] ?? '', '/')), fn($s) => $s !== ''));
        if (count($segments) < 2) return null;
        if (end($segments) === 'support') array_pop($segments);
        $this->productInfo = [
    ...
    {
        if ($this->pageContent !== null) return $this->pageContent;
        $info = $this->getProductInfo();
        if (!$info) return null;
        try {
            $this->pageContent = getContents(
                sprintf('https://www.gigabyte.com/%s/%s/support', $info['category'], $info['product']),
                ['Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8', 'User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36']
            );
    ...
    private function normalize(string $text, bool $keepLinks = false): string
    {
        ...
        if ($keepLinks) {
            ...
            $text = preg_replace_callback('/<a\s+([^>]*?)>/is', function ($m) use ($linkStyle) {
                ...
                if (stripos($attrs, 'style=') === false) $attrs .= ' style="' . $linkStyle . '"';
                return '<a ' . trim($attrs) . '>';
            }, $text);
    ...
    {
        if (!preg_match('/href="([^"]*(?:download\.gigabyte\.com|\.zip)[^"]*)"/i', $row, $match)) return '';
        $url = $match[1];
    ...
    {
        if (stripos($row, '<th') !== false) return null;
    ...
    private function extractRowsFromTable(string $tableHtml): array
    {
        ...
        foreach ($matches[1] as $row) {
            ...
            if ($hasOs === null && count($cellMatches[1]) > 0) $hasOs = count($cellMatches[1]) >= 6;
    ...
    {
        $itemTitle = $data['type'] === 'bios'
            ? sprintf('[%s] %s', $data['category'], $data['version'])
            : preg_replace('/\s+/', ' ', trim(trim(preg_replace('/Checksum\s*:\s*\S+/i', '', sprintf('[%s] %s', $data['category'], strip_tags($data['description'])))))) . (!empty($data['version']) ? ' - ' . $data['version'] : '');
        $uri = sprintf('https://www.gigabyte.com/%s/%s/support', $info['category'], $info['product']) . (!empty($info['fragment']) ? '#' . $info['fragment'] : '');
        $content = '<div style="' . self::CSS['item'] . '">'
            . $this->render('Description', $data['description'], true, true)
            . $this->render('OS', $data['os'])
            . $this->render('Size', $data['size'])
            . (!$hideAttachments && !empty($data['download']) ? sprintf('<p style="%s"><a href="%s" style="%s" target="_blank" rel="noopener noreferrer">Download</a></p>', self::CSS['p'], htmlspecialchars($data['download'], ENT_QUOTES, 'UTF-8'), self::CSS['download']) : '')
            . '</div>';
        $item = ['title' => $itemTitle, 'uri' => $uri, 'content' => $content, 'uid' => md5($itemTitle . $data['version'])];
        if (!empty($data['date']) && ($timestamp = strtotime($data['date'])) !== false) $item['timestamp'] = $timestamp;
        return $item;
    }
    ...
    {
        $info = $this->getProductInfo();
        if (!$info || !$info['product'] || !$info['category']) throwClientException('Invalid URL format or could not extract product info. Expected format: https://www.gigabyte.com/Category/Product-ID/support');
        $html = $this->fetchPageContent();
        if (!$html) throwClientException('Failed to fetch support page content');
        ...
        foreach ($this->parseSections($html) as $item) {
            if (in_array($item['type'], $types, true)) $allItems[] = $this->buildFeedItem($item, $info, $hideAttachments);
        }
        if (empty($allItems)) return;
        usort($allItems, fn($a, $b) => ($b['timestamp'] ?? 0) <=> ($a['timestamp'] ?? 0));
        foreach ($allItems as $item) $this->items[] = $item;
    }
}