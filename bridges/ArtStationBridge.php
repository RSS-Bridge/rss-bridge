...
class ArtStationBridge extends BridgeAbstract {
	...
	public function fetchSearch($searchQuery) {
		$data = '{"query":"' .$searchQuery. '","page":1,"per_page":50,"sorting":"date","pro_first":"1","filters":[],"additional_fields":[]}';
	...
	public function collectData() {
		...
		foreach($jsonQuery->data as $media)
		{
			...
			$item['content'] = 
				'<a href="' .$media->url. '">
				<img style="max-width: 100%" src="' . $jsonProject->cover_url . '">
				</a>' . $jsonProject->description;
				
			$numAssets = count($jsonProject->assets);

			if ($numAssets > 1)
				$item['content'] .= '<p><a href="' .$media->url. '">Project contains ' . ($numAssets - 1) . ' more item(s).</a></p>';
