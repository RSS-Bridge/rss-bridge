...
class NginxBridge extends BridgeAbstract
{
    const NAME = 'Nginx';
    ...
    private const BASE_URL = 'https://nginx.org';
    ...
    {
        $content = getContents('https://nginx.org/en/CHANGES');
        if (!$content) {
            throw new \Exception('Failed to load CHANGES');
        }

        $blocks = preg_split('/(?=Changes with nginx\s+)/', $content);
        ...
        foreach ($blocks as $block) {
            ...
            if (!preg_match('/^Changes with nginx\s+(\S+)\s+(\d{1,2}\s+\w+\s+\d{4})/', $block, $m)) {
                continue;
            }

            $version = $m[1];
            $changes = preg_replace('/^Changes with nginx\s+\S+\s+\d{1,2}\s+\w+\s+\d{4}\s*/', '', $block);
            ...
            $this->items[] = [
                'uri' => 'https://nginx.org/en/CHANGES',
                'title' => "NGINX {$version}",
                'content' => $html,
    ...
    {
        $html = getSimpleHTMLDOM('https://nginx.org/news.html');
        if (!$html) {
            throw new \Exception('Failed to load news.html');
        }
    ...
    {
        $source = $this->getInput('source');
        return $source === 'news' ? 'NGINX News' : 'NGINX Releases';
    }
