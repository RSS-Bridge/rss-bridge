...
final class Fetcher
{
    ...
    public function getContents(
        ...
        $result = $this->httpRequest($url, $config);
        $response = [
            'code' => $result['code'],
            ...
            default:
                throw new HttpException(
                    sprintf(
                        '%s %s',
                        $result['code'],
                        self::HTTP_STATUS_CODES[$result['code']] ?? ''
                    ),
                    $result['code']
                );
        }
     ...
     */
    private function httpRequest(string $url, array $config = []): array
    {
        $defaults = [
     ...
     * @return false|simple_html_dom Contents as simplehtmldom object.
     */
    public function getSimpleHTMLDOMCached(
        $url,
        $duration = 86400,
        $header = [],
        $opts = [],
