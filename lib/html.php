...
function handleYoutube(string $string)
{
    ...
    if ($useIframe) {
        ...
        return sprintf('<iframe width="560" height="315" src="%s" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin" allowfullscreen></iframe>', $embedUri);
    } else {
        $videoUri = 'https://www.youtube.com/watch?v=' . $videoID;

        $thumbnailJpegBaseUri = 'https://i.ytimg.com/vi/' . $videoID;
        $jpegSrcset = sprintf('%1$s/mqdefault.jpg 320w, %1$s/0.jpg 480w, %1$s/hqdefault.jpg 481w, %1$s/sddefault.jpg 640w, %1$s/hq720.jpg 720w, %1$s/maxresdefault.jpg 721w', $thumbnailJpegBaseUri);

        $thumbnailWebpBaseUri = 'https://i.ytimg.com/vi_webp/' . $videoID;
        $webpSrcset = sprintf('%1$s/mqdefault.webp 320w, %1$s/0.webp 480w, %1$s/hqdefault.webp 481w, %1$s/sddefault.webp 640w, %1$s/hq720.webp 720w, %1$s/maxresdefault.webp 721w', $thumbnailWebpBaseUri);

        $fallbackUri = $thumbnailJpegBaseUri . '/maxresdefault.jpg';

        return sprintf('<a href="%s"><picture><source srcset="%s" type="image/webp" referrerpolicy="no-referrer" /><img srcset="%s" src="%s" alt="Video thumbnail" title="YouTube video thumbnail" referrerpolicy="no-referrer" /></picture></a>', $videoUri, $webpSrcset, $jpegSrcset, $fallbackUri);
    }
}
